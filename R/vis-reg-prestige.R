...
# slopes, b +- se / sx
show.slope(x=14, y=int+14*slope, b=slope, cex=1.2)
bup <- slope + sigma / sx
int.up <- my - bup * mx
show.slope(x=14, y=75.343, b=bup, label=~b+frac(s[e],s[x]), unit=FALSE, cex=1.2)

blo <- slope - sigma / sx
show.slope(x=14, y=53.67304, b=blo, label=~b-frac(s[e],s[x]), unit=FALSE, cex=1.2)

lines(bse<-brace(x1=mx, y1=my, x2=mx, y2=my+sigma, right=FALSE, rad=0.15))
text(min(bse[,1]), my+sigma/2, label=~s[e], cex=1.2, adj=c(1,.5))

lines(bsx <- brace(x1=mx, y1=my-sigma, x2=bbox[1,1], y2=my-sigma, rad=0.15))
text(mx+sx/2, min(bsx[,2]), label=~s[x], adj=c(0.5, 1), cex=1.2)

#text(7, 80, label=~b %+-% frac(s[e], s[x]), cex=1.5, adj=c(0, 0.5))
text(7, 80, label=~b %+-% group("(", frac(s[e], s[x]),")"), cex=1.5, adj=c(0, 0.5))
...
text(7, 80, label=~b %+-% frac(2, sqrt(n)) * group("(", frac(s[e], s[x]),")"), cex=1.5, adj=c(0, 0.5))
