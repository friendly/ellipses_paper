...
The simplest motivating example is two-group discriminant analysis (\secref{sec:discrim}).
...
the \emph{locus of osculation}, whose properties are described in \secref{sec:locus}.
...
for method B, such as ridge regression (\secref{sec:ridge}) or Bayesian estimation
(\secref{sec:bayesian}). The remarkable fact is that the geometry of such kissing
...
\subsection{Locus of osculation}\label{sec:locus}
...
An exact general solution can be described as follows:  Let the ellipses for $i=1, 2$ be given by
% \begin{eqnarray*}
...
\end{equation}
\eqref{eq:locus} is a bilinear form in \vec{x}, with central matrix $\mat{A}_2 \trans \: \mat{C} \: \mat{A}_1$,
